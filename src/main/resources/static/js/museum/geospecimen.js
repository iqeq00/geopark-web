...
layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'laydate','imageUpload'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            //{field: 'fieldnumber', align: 'center', sort: true, title: '野外编号'},
            {field: 'hallnumber', align: 'center', sort: true, title: '馆内编号'},
            //{field: 'englishlatin', align: 'center', sort: true, title: '英文或拉丁学名'},
            ...
            //{field: 'identificationhuman', align: 'center', sort: true, title: '鉴定人'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/geospecimen/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                imageUpload.init("uploadBtn", "img", "geospecimen");
                if (data) {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/geospecimen/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/geospecimen', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
