...
layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'laydate','imageUpload'], function () {
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/humenexhibit/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                imageUpload.init("uploadBtn", "img", "humenexhibit");
                if (data) {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/humenexhibit/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/humenexhibit', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
