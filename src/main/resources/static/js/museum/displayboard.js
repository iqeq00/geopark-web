...
layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'laydate','imageUpload'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            {field: 'boardnumber', align: 'center', sort: true, title: '展板编号'},
            {field: 'hallname', align: 'center', sort: true, title: '展厅名称'},
            {field: 'boardcontent', align: 'center', sort: true, title: '展板内容'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/displayboard/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                imageUpload.init("uploadBtn", "img", "displayboard");
                if (data) {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/displayboard/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/displayboard', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
