layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'imageUpload'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            {field: 'docname', align: 'center', sort: true, title: '文件名称'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/picdocs/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                imageUpload.init("uploadBtn", "url", "picdocs");
                if (data) {
                    form.val('formFilter', data);
                    imageUpload.initImageList(data.url);
                }
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/picdocs/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/picdocs', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
