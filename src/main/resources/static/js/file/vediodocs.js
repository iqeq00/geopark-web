layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'fileUpload'], function () {
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/vediodocs/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                fileUpload.init("uploadBtn", "url", "vediodocs");
                if (data) {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/vediodocs/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/vediodocs', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
