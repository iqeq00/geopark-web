...
layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'laydate'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            {field: 'ethnic', align: 'center', sort: true, title: '公园内人口民族情况'},
            {field: 'areaethnic', align: 'center', sort: true, title: '所在区域人口民族情况'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/socialeconomy/' + obj.data.id, {}, function () {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/socialeconomy/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/socialeconomy', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
