layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'imageUpload'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            {field: 'title', align: 'center', sort: true, title: '会议标题'},
            {field: 'address', align: 'center', sort: true, title: '地点'},
            {field: 'startAt', align: 'center', sort: true, title: '时间'},
            {field: 'delegate', align: 'center', sort: true, title: '参会代表'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/parkmeeting/' + obj.data.id, {}, function () {
    ...
    var showEditModel = function (data) {
        layer.open({
            ...
            success: function () {
                ...
                imageUpload.init("uploadBtn", "img", "parkmeeting");
                if (data) {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/parkmeeting/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/parkmeeting', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
