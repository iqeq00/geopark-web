...
layui.use(['config', 'lichee', 'jquery', 'layer', 'table', 'form', 'laydate'], function () {
    ...
    var tableInfo = table.render({
        ...
        cols: [[
            ...
            {field: 'samplenumber', align: 'center', sort: true, title: '样品编号'},
            {field: 'sampletype', align: 'center', sort: true, title: '样品类型'},
            {field: 'sampleposition', align: 'center', sort: true, title: '采样位置'},
    ...
    table.on('tool(tableFilter)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('确定要删除吗？', function (index) {
                layer.load(2);
                lichee.delete('/sampling/' + obj.data.id, {}, function () {
    ...
    form.on('submit(formSubmit)', function (data) {
        layer.load(2);
        if (data.field.id) {
            lichee.put('/sampling/' + data.field.id, {data: data.field}, function (res) {
                callFunction(res);
            });
        } else {
            lichee.post('/sampling', {data: data.field}, function (res) {
                callFunction(res);
            });
        }
        return false;
